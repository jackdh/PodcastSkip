...
Force update in Settings unregisters the service worker, clears the app-shell cache (downloaded episodes are kept), and reloads from the network. That is required for iOS Home Screen apps, which otherwise keep serving the old precache even after a reload. A waiting build still shows a Reload banner instead of refreshing by itself. If the version date still does not change, fully quit the Home Screen app from the app switcher and reopen it.
